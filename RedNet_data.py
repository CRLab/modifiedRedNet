...
class GibsonDataset(Dataset):
    def __init__(self, transform=None, phase_train=True, data_dir=None):
        ...
        try:
            with open(data_dir + img_dir_train_file.split('/')[-1], 'r') as f:
                self.img_dir_train = f.read().splitlines()
            with open(data_dir + depth_dir_train_file.split('/')[-1], 'r') as f:
                self.depth_dir_train = f.read().splitlines()
            with open(data_dir + label_dir_train_file.split('/')[-1], 'r') as f:
                self.label_dir_train = f.read().splitlines()
            with open(data_dir + img_dir_test_file.split('/')[-1], 'r') as f:
                self.img_dir_test = f.read().splitlines()
            with open(data_dir + depth_dir_test_file.split('/')[-1], 'r') as f:
                self.depth_dir_test = f.read().splitlines()
            with open(data_dir + label_dir_test_file.split('/')[-1], 'r') as f:
                self.label_dir_test = f.read().splitlines()

            def fix_paths(files):
                return [file.replace('data/gibson_data/', data_dir).replace(' ', '') for file in files]
            
            self.img_dir_train = fix_paths(self.img_dir_train)
            self.depth_dir_train = fix_paths(self.depth_dir_train)
            self.label_dir_train = fix_paths(self.label_dir_train)
            self.img_dir_test = fix_paths(self.img_dir_test)
            self.depth_dir_test = fix_paths(self.depth_dir_test)
            self.label_dir_test = fix_paths(self.label_dir_test)

            print("Loaded a Gibson Dataset!")
        except:
